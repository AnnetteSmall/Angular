  ...
  private initForm() {
    ...
    if (this.editMode) {
      ...
      if (recipe['ingredients']) {
        for (let ingeredient of recipe.ingredients) {
          recipeIngredients.push(
            new FormGroup({
              'name': new FormControl(ingeredient.name, Validators.required),
              'amount': new FormControl(ingeredient.amount, [Validators.required, Validators.pattern(/^[1-9]+[0-9]*$/)
              ])
            })
          );
        }
